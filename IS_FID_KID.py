...
def inception_score():
    filenames = glob(os.path.join('test_sample/ffhq_sunglasses', '*.*'))
...
def mean_kernel_inception_distance():
    ...
    filenames = glob(os.path.join('2S1_train', '*.*'))
    ...
    filenames = glob(os.path.join('2S1_test', '*.*'))
    fake_images = [get_images(filename) for filename in filenames]
    fake_images = np.transpose(fake_images, axes=[0, 3, 1, 2])
