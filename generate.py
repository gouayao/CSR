...
def generate_imgs(args, g_list, device, mean_latent):
    ...
    #
    #     utils.save_image(tot_img, f'samples/ffhq_sketches1.png', nrow=5, normalize=True, range=(-1, 1), )

    #     for i in range(args.n_sample):
    #         utils.save_image(tot_img[i], f'test_sample/fake_sketches_new2/' + str(i) + '.png', normalize=True, range=(-1, 1), )
        ...
        for j in range(args.n_sample):
            for i in range(len(g_list)):
                ...
                if i == 0:
                    ...
                    tot_img = torch.cat([tot_img, sample], dim = 0)
            utils.save_image(tot_img, f'test_sample/ffhq_sunglasses/' + str(j) + '.png', nrow=1, normalize=True, range=(-1, 1), )
