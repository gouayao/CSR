...
from torch.nn import functional as F, L1Loss
...
####contrastive loss
def mcl(out1, out2, temperature, distributed=False):
    if distributed:
...
def train(args, loader, generator, discriminator, extra, g_optim, d_optim, e_optim, g_ema, device, g_source, d_source):
    ...
    for idx in pbar:
        ...
        #####aug
        real_new = torch.zeros([4,3,256,256])

        for pair1 in range(args.feat_const_batch):
            data_transform = transforms.Compose([transforms.Normalize(mean = (-1, -1, -1), std = (2, 2, 2)), transforms.ToPILImage(), transforms.RandomHorizontalFlip(p=1), transforms.ToTensor(), transforms.Normalize(mean = (0.5, 0.5, 0.5), std = (0.5, 0.5, 0.5))])
            real_new[pair1] = data_transform(real_img[pair1])


        # utils.save_image(real_img, './sample.png', nrow=1, normalize=True, range=(-1, 1), )
        # utils.save_image(real_new, './samples.png', nrow=1, normalize=True, range=(-1, 1), )

        real_img = real_img.to(device)
        real_new = real_new.to(device)
        ...
        if args.augment:
            ...
            real_new, _ = augment(real_new, ada_aug_p)
        ...
        new_pred, _ = discriminator(
            real_new, extra=extra, flag=which, p_ind=np.random.randint(lowp, highp), real=True)


        #discriminative contrastive loss
        if which > 0:
            # loss_mcl
            # mcl_fake = F.normalize(fake_pred.view(-1, 32))
            mcl_real = F.normalize(real_pred.view(-1, 32))
            mcl_new = F.normalize(new_pred.view(-1, 32))
            mcl_fake1 = F.normalize(fake_pred.view(-1, 128))
            mcl_real1 = F.normalize(real_pred.view(-1, 128))
            mcl_new1 = F.normalize(new_pred.view(-1, 128))
            # d_mcl = mcl(mcl_fake, mcl_real, temperature = args.temp, distributed=False)
            d_sup = supcon_fake(mcl_real1, mcl_new1, mcl_fake1, temperature = args.temp, distributed=False)
            simclr_loss = nt_xent(mcl_real, mcl_new, temperature = args.temp, distributed=False, normalize=False)
            # print('d_sup:', d_sup)
            # print('simclr_loss:', simclr_loss)
            d_loss = d_logistic_loss(real_pred, fake_pred) + 0.05 * d_sup + 0.05 * simclr_loss
        else:
        ...
        target_sample, feat_target = generator([z], return_feats=True)
        ...
        source_target = torch.zeros(
            [args.feat_const_batch, args.feat_const_batch - 1]).cuda()
        target_source = torch.zeros(
            [args.feat_const_batch, args.feat_const_batch - 1]).cuda()

        l = torch.zeros([args.feat_const_batch]).cuda()
        for pair1 in range(args.feat_const_batch):
            tmpc = 0

            l[pair1] = loss_fn_vgg(source_sample[pair1], target_sample[pair1])
            for pair2 in range(args.feat_const_batch):  # comparing the possible pairs
                if pair1 != pair2:
                    anchor_source = torch.unsqueeze(
                        feat_source[feat_ind[pair1]][pair1].reshape(-1), 0)
                    compare_target = torch.unsqueeze(
                        feat_target[feat_ind[pair1]][pair2].reshape(-1), 0)
                    source_target[pair1, tmpc] = sim(anchor_source, compare_target)

                    anchor_target = torch.unsqueeze(
                        feat_target[feat_ind[pair1]][pair1].reshape(-1), 0)
                    compare_source = torch.unsqueeze(
                        feat_source[feat_ind[pair1]][pair2].reshape(-1), 0)
                    target_source[pair1, tmpc] = sim(anchor_target, compare_source)

                    tmpc += 1
        source_target = sfm(source_target)
        target_source = sfm(target_source)
        new_loss = args.kl_wt * \
            kl_loss(torch.log(source_target), target_source) # distance consistency loss

        # l1loss = L1(source_sample, target_sample).mean()

        #new(csr loss) and perceptual similarity loss
        g_loss = g_loss + 0 * rel_loss + 0.1 * new_loss + 0.1 * l.mean()

        # g_loss = g_loss + 1 * rel_loss + 0 * new_loss + 0 * l.mean()
